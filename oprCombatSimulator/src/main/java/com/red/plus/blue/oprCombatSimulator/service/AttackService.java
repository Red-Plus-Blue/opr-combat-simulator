...
public class AttackService {
    ...
    public Stream<WoundGroup> attackWithWeaponGroup(final Unit attacker, final Unit defender, WeaponGroup weaponGroup) {
        ...
        return getHits(attacker, weaponGroup)
                .map(toHit)
                // apply hit flags (ex. rending => DISABLE_REGEN)
                .map(hit -> specialRulesService.applyHitFlags(specialRules, hit))
                // apply hit multipliers (ex. blast, v2.5 poison, etc.)
                .flatMap(hit -> specialRulesService.applyHitMultipliers(specialRules, hit))
                ...
                // apply modifiers to the defender (ex. ap, rending)
